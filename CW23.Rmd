...
# suppress warnings
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, error = FALSE, message = FALSE, fig.align = 'center', eval = TRUE)
```
...
We can see that $\boldsymbol{\hat\lambda}$ is very sensitive to $N$ at smaller values (up to the power of 3) but then becomes fairly consistent as $N$ grows larger.
...
```{r}
J_g <- grad_fn(theta = fit_q2$par, y_sample_q1, N_val)
...
$$f(y|\lambda_1,\lambda_2)=\exp(-\lambda_2y+\lambda_1\log\lambda_2+\log\Gamma(\lambda_1)+\lambda_1\log y)=\exp(\frac{\frac{\lambda_2}{\lambda_1}y-\log\lambda_2}{-\lambda_1^{-1}}+\log\Gamma(\lambda_1)+\lambda_1\log y)$$
And so we can reparametrise $\mu=\frac{\lambda_2}{\lambda_1}$, $\phi=\frac{1}{\lambda_1}$ to get the following:
$$\exp(\frac{\mu y-\log\mu}{-\phi}+\frac{\log\phi}{\phi}+(\phi^{-1}-1)\log{y}-\log\Gamma\phi^{-1})$$

And in fact, we know that $\mathbb E[Y] = \frac{1}{\mu}$ by the properties of the Gamma distribution.
...
fit_gamma_model <- function(p = 2, link = 'inverse', y = data_q7$y, x = data_q7$x, n_starts = 100) {
  ...
  log_dens_gamma_expr <- expr(
    - (y * !!mu_expr) / exp(theta1)
    + log(!!mu_expr) / exp(theta1)
    + theta1 / exp(theta1)
    - (1 - 1 / exp(theta1)) * log(y)
    - lgamma(1 / exp(theta1))
  )
...
Comparing the two models, we see that the Normal distribution has smaller confidence intervals and actually seems to extrapolate to $(-3,3)$ better. However, when we select the best model using AIC, we actually choose the Gamma model:

```{r}
calc_aic(best_norm_fit$model, best_norm_fit$d) < calc_aic(best_gamma_fit$model, best_gamma_fit$d)
```
